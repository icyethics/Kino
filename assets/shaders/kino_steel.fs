...
extern MY_HIGHP_OR_MEDIUMP vec2 kino_steel;
...
#define MAX_ALLOWED_COLOURS 8

vec3 allowed_colours[4] = vec3[4](vec3(0.631, 0.667, 0.671), vec3(0.773, 0.792, 0.792), vec3(0.486, 0.535, 0.543), vec3(0.301, 0.387, 0.402));
int allowed_colour_count = 4;
float colour_tolerance = 0.03; // e.g. 0.05
...
bool isAllowedColour(vec3 c)
{
    for (int i = 0; i < MAX_ALLOWED_COLOURS; i++)
    {
        if (i >= allowed_colour_count) break;

        if (distance(c, allowed_colours[i]) <= colour_tolerance)
            return true;
    }
    return false;
}
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    MY_HIGHP_OR_MEDIUMP number fac = 0.8 + 0.9*sin(13.*uv.x+5.32*uv.y + kino_steel.r*12. + cos(kino_steel.r*5.3 + uv.y*4.2 - uv.x*4.));
    MY_HIGHP_OR_MEDIUMP number fac2 = 0.5 + 0.5*sin(10.*uv.x+2.32*uv.y + kino_steel.r*5. - cos(kino_steel.r*2.3 + uv.x*8.2));
    MY_HIGHP_OR_MEDIUMP number fac3 = 0.5 + 0.5*sin(12.*uv.x+6.32*uv.y + kino_steel.r*6.111 + sin(kino_steel.r*5.3 + uv.y*3.2));
    MY_HIGHP_OR_MEDIUMP number fac4 = 0.5 + 0.5*sin(4.*uv.x+2.32*uv.y + kino_steel.r*8.111 + sin(kino_steel.r*1.3 + uv.y*13.2));
    MY_HIGHP_OR_MEDIUMP number fac5 = sin(0.5*16.*uv.x+5.32*uv.y + kino_steel.r*12. + cos(kino_steel.r*5.3 + uv.y*4.2 - uv.x*4.));
