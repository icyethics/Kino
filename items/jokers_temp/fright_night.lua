SMODS.Joker {
    ...
    locked_loc_vars = function(self, info_queue, card)
        return {
            vars = {
                G.PROFILES[G.SETTINGS.profile].career_stats.kino_blood_counters_drained or 0,
                20
            }
        }
    end,
    check_for_unlock = function(self, args)
        if args.type == 'career_stat' then
            if G.PROFILES[G.SETTINGS.profile].career_stats.kino_blood_counters_drained and G.PROFILES[G.SETTINGS.profile].career_stats.kino_blood_counters_drained >= 20 then
                unlock_card(self)
            end
        end
    end,
