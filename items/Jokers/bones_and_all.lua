SMODS.Joker {
    ...
    k_genre = {"Romance", "Fantasy", "Horror"},
    ...
    locked_loc_vars = function(self, info_queue, card)
        return {
            vars = {
                G.PROFILES[G.SETTINGS.profile].career_stats.kino_romance_jokers_destroyed or 0,
                5
            }
        }
    end,
    check_for_unlock = function(self, args)
        if args.type == 'career_stat' then
            if G.PROFILES[G.SETTINGS.profile].career_stats.kino_romance_jokers_destroyed and G.PROFILES[G.SETTINGS.profile].career_stats.kino_romance_jokers_destroyed >= 5 then
                unlock_card(self)
            end
        end
    end,
